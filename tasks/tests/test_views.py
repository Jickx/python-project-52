...
class TaskViewsTests(TestCase):
    ...
    def test_delete_only_author_can_delete(self):
        """Non-author cannot delete task created by another user"""
        self.client.login(username='u2', password='pass12345')
        url = reverse('tasks:delete', args=[self.task.pk])

        # GET should redirect with error
        resp_get = self.client.get(url, follow=True)
        self.assertRedirects(resp_get, reverse('tasks:list'))
        msgs = list(get_messages(resp_get.wsgi_request))
        self.assertTrue(any('author' in str(m).lower() for m in msgs))

        # POST should also redirect without deleting
        resp_post = self.client.post(url, follow=True)
        self.assertRedirects(resp_post, reverse('tasks:list'))
        self.assertTrue(Task.objects.filter(pk=self.task.pk).exists())

    def test_author_can_delete_own_task(self):
        """Author can delete their own task"""
        self.client.login(username='u1', password='pass12345')
        to_delete = Task.objects.create(
            name='Author task',
            description='Test',
